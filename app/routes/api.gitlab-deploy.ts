...
async function gitlabDeployHandler({ request }: { request: Request }) {
    ...
    try {
        ...
        try {
            ...
            const groupProjectsResponse = await fetch(`${gitlabUrl}/api/v4/groups/${groupId}/projects?search=${sanitizedName}`, {
            ...
            if (!response.ok) {
                ...
                try {
                    ...
                    if (errorData.message?.namespace?.[0]?.includes('is not valid')) {
                        errorMessage = 'Failed to create project: The configured GITLAB_GROUP_ID is either invalid or your GITLAB_ACCESS_TOKEN does not have Developer/Maintainer permissions to create projects in this group.';
                    } else {
        ...
        if (files && Object.keys(files).length > 0) {
            if (!projectExists) {
                ...
                await apiService.createProjectWithFiles(sanitizedName, isPrivate, files).catch(async () => {
                    ...
                    await apiService.commitFiles(project.id, { branch: 'main', commit_message: 'Initial commit from Devonz', actions });
                });
            } else {
